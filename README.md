...
RPCGateway 是整个微服务架构的**流量枢纽**。职责：协议转换（HTTP↔Protobuf）、路由分发、Token 鉴权、**TLS termination**。基于 C++ epoll 事件驱动，单机支撑数万并发。TLS 证书由 CertService 独立分发，启动时调用 `DistributeCert` 获取。
