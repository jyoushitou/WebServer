...
│   └── 简介：统一管理核心基础设施的 Web 控制台，提供服务状态监控、配置管理、健康检查等运维功能
│
├── ServiceConsole/                     # [子仓库] 业务服务管理面板 (C++) ✅ 核心
│   └── 管理用户/文章/博客/图片/视频/搜索的独立 Web UI 面板
│   └── 简介：统一管理业务数据的 Web 控制台，提供用户管理、内容审核、运营统计等管理功能
│
├── GRPCGateway/                        # [子仓库] 网关 (C++) ✅ 核心
│   └── 对外统一入口，协议转换，路由分发，限流控制，鉴权验证
│   └── 简介：所有外部请求的唯一入口，负责 HTTP/gRPC 协议转换、请求路由、限流熔断、身份鉴权，是微服务架构的流量枢纽
│
├── ServiceRegistry/                    # [子仓库] 服务注册发现 (C++) ✅ 核心
│   └── 动态路由，负载均衡，健康检查，服务上下线
│   └── 简介：维护所有微服务的动态注册表，提供服务发现、健康检测、负载均衡、上下线通知，保障服务间高效稳定的通信
│
├── ConfigCenter/                       # [子仓库] 配置中心 (C++) ✅ 核心
│   └── 统一配置管理，热更新，配置版本控制
│   └── 简介：集中管理所有微服务的配置项，支持运行时热更新、版本回溯、变更推送，避免配置散落和重启部署
│
├── TracingService/                     # [子仓库] 链路追踪 (C++) ✅ 核心
│   └── 请求链路追踪，性能分析，故障排查
│   └── 简介：采集分布式请求的调用链数据，可视化展示服务依赖拓扑，快速定位性能瓶颈和异常节点
│
├── MonitorService/                     # [子仓库] 监控告警 (C++) ✅ 核心
│   └── 指标采集，告警规则，可视化面板
│   └── 简介：实时采集 CPU/内存/QPS/延迟等服务指标，配置告警规则，出现异常时自动通知运维人员
│
├── UserService/                        # [子仓库] 用户服务 (Rust)
│   └── 用户注册/登录、Token 管理、权限控制、设备管理
│   └── 简介：安全敏感的用户认证中心，处理用户注册登录、Token 签发验证、权限角色管理，利用 Rust 内存安全特性保障用户数据安全
│
├── ArticleService/                     # [子仓库] 文章服务 (Go)
│   └── 文章 CRUD、分类管理、标签管理、文章搜索
│   └── 简介：内容管理系统核心，提供文章增删改查、分类标签维护，Go 的高并发协程模型轻松应对大量读写请求
│
├── BlogService/                        # [子仓库] 博客服务 (Go)
│   └── 博客管理、评论系统、点赞收藏、博客推荐
│   └── 简介：社交化博客平台，支持博客发布、评论互动、点赞收藏、个性化推荐，Go 的 goroutine 高效处理大量并发互动请求
│
├── ImageService/                       # [子仓库] 图片服务 (C++)
│   └── 图片上传、缩略图生成、格式转换、CDN 分发
│   └── 简介：高性能图片处理引擎，基于 C++ 实现像素级图像处理，支持批量缩略图生成、格式转换，极致利用 CPU 多核性能
│
├── VideoService/                       # [子仓库] 视频服务 (Go)
│   └── 视频上传、转码任务调度（FFmpeg）、切片存储、流媒体播放
│   └── 简介：视频处理调度中心，用 Go 编排 FFmpeg 转码任务队列，管理视频切片和流媒体分发，Go 的并发模型完美适配任务调度场景
│
├── SearchService/                      # [子仓库] 搜索服务 (C++)
│   └── 全文索引、搜索排序（ES 集成）、搜索建议、索引同步
│   └── 简介：高性能搜索引擎，封装 Elasticsearch 提供全文检索、排序评分、智能建议，C++ 实现查询解析与排序算法保证毫秒级响应
│
├── vue/                                # [子仓库] Vue 3 前端
│   └── 用户界面、gRPC-Web 通信、实时数据展示
│   └── 简介：用户端 Web 应用，基于 Vue 3 + TypeScript 构建，通过 gRPC-Web 与后端通信，提供文章浏览、视频播放、博客互动等丰富交互体验
│
├── mobile/                             # [子仓库] 移动端 App
│   ├── ios/                            # iOS App (Swift) — 用户端，浏览文章/博客/图片/视频
│   └── android/                        # Android App (Kotlin) — 用户端，浏览文章/博客/图片/视频
│   └── 简介：原生移动端应用，iOS 用 Swift、Android 用 Kotlin，通过 gRPC 直接连接后端服务，提供流畅的移动端内容消费体验
│
├── assets/                             # [子仓库] 设计资源
...
│   ├── logos/                          # Logo 资源
│   └── mockups/                        # 产品原型 / 交互稿
│   └── 简介：全平台设计资产仓库，包含 Figma 设计稿、图标库、Logo 品牌资源、产品原型，统一管理前端和移动端的设计交付物
│
